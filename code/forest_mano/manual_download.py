...
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) # os.getcwd() for juypter notebook
...
download_dest_folder_name = config["raw_data_folder"]

# set the download destination directory
direc = raw_data_dir
print(f"Resolved absolute path: {direc}", flush=True)

dest_folder_name = download_dest_folder_name
server = "studies"

task_id = int(sys.argv[1]) # 1 to test
